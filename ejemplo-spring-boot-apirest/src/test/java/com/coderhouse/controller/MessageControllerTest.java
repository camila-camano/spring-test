...
public class MessageControllerTest {
    ...
    @Test
    public void getMessageByDescription() throws  Exception{
        String desc_first= "Mensaje-ABCD";
        System.out.println(desc_first);
        var result = mockMvc.perform(get("/coder-house/mensajes").param("description","Mensaje-ABCD"))
                .andDo(print())
                .andExpect(status().isOk())
                .andReturn();

        var list = result.getResponse().getContentAsString();
        var messages = mapper.readValue(list, List.class);

        Assert.notNull(messages, "ERROR: Lista recibida nula.");
        Assert.isTrue(messages.size() == 3, "ERROR: Tamaño de la lista esperada es de 3, la lista obtenida es diferente.");

        for(Object elem : messages){
            Message message = mapper.convertValue(elem, Message.class);
            Assert.isTrue(message.getDescription().equals(desc_first), "ERROR: descripción del elemento no es igual a la pedida.");
        }
        System.out.println("ÉXITO: El resultado no es nulo y hay match de todas las descripciones de los elementos de la lista con la pedida.");
    }


    @Test
    public void getExampleMessage() throws Exception {
        mockMvc.perform(get("/coder-house/mensajes/example"))
                .andDo(print())
                .andExpect(status().isOk())
